...
class UploadFile {//类定义开始
    ...
    public function upload($savePath ='') {
        ...
        if($isUpload) {
            ...
            //$this->error  =  '没有选择上传文件';
            $this->error  =  'No option to upload files!';
            return false;
        }
    }
    ...
    public function uploadOne($file,$savePath=''){
        ...
        if(!is_dir($savePath)) {
            ...
            if(!mkdir($savePath,0777,true)){
                //$this->error  =  '上传目录'.$savePath.'不存在';
                $this->error  =  'Upload directory'.$savePath.'inexistence!';
                return false;
            }
        }else {
            if(!is_writeable($savePath)) {
    ...
    private function check($file) {
        ...
        if(!$this->checkSize($file['size'])) {
            //$this->error = '上传文件大小不符！';
            $this->error = 'Upload file size discrepancy!';
            return false;
        }
        ...
        if(!$this->checkType($file['type'])) {
            ...
            $this->error = 'The MIME type is not allowed to upload files!';
            return false;
        }
        //检查文件类型
        if(!$this->checkExt($file['extension'])) {
            //$this->error ='上传文件类型不允许';
            $this->error ='Upload the file type is not allowed!';
            return false;
        }
